...
There are some many options to handle state in react applications. Obviously you can use `setState` for some small logic, but what if you have a complex state to manage?
...
You can initialize a context api as simple as that, with any value you want, in this case I'm using and empty object.
...
Every product inside the product list is going to have an id, name and a price.
...
Also, we create the `AppProvider` component, and inside this, the `useReducer` hook take this `mainReducer` and the initial state to return the `state` and the `dispatch`.
...
Everything now it's strongly typed.
